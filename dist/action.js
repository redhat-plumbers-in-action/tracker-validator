...
async function action(octokit, owner, repo, prMetadata) {
    ...
    if (!trackerController.adapter.isMatchingProduct(product)) {
        ...
        err.push(`🔴 Bugzilla tracker ${trackerController.adapter.getMarkdownUrl()} has product \`${issueDetails.product}\` but desired product is \`${product}\``);
    }
    else {
        if (labelsFromPR.includes('bz/invalid-product')) {
            removeLabel(octokit, owner, repo, prMetadata.number, 'bz/invalid-product');
        }
        message.push(`🟢 Bugzilla tracker ${trackerController.adapter.getMarkdownUrl()} has set desired product: \`${product}\``);
    }
    ...
    if (!trackerController.adapter.isMatchingComponent(component)) {
        ...
        err.push(`🔴 Bugzilla tracker ${trackerController.adapter.getMarkdownUrl()} has component \`${issueDetails.component}\` but desired component is \`${component}\``);
    }
    else {
        if (labelsFromPR.includes('bz/invalid-component')) {
            removeLabel(octokit, owner, repo, prMetadata.number, 'bz/invalid-component');
        }
        message.push(`🟢 Bugzilla tracker ${trackerController.adapter.getMarkdownUrl()} has set desired component: \`${component}\``);
    }
    if (!trackerController.adapter.isApproved()) {
        labels.add.push('bz/unapproved');
        err.push(`🔴 Bugzilla tracker ${trackerController.adapter.getMarkdownUrl()} has not been approved`);
    }
    else {
        if (labelsFromPR.includes('bz/unapproved')) {
            removeLabel(octokit, owner, repo, prMetadata.number, 'bz/unapproved');
        }
        message.push(`🟢 Bugzilla tracker ${trackerController.adapter.getMarkdownUrl()} has been approved`);
    }
